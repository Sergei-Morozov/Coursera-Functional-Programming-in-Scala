...
object Huffman {
  ...
  // Part 1: Basics
    ...
    def chars(tree: CodeTree): List[Char] = tree match {
      ...
      case Leaf(char: Char, weight: Int) => List[Char](char)
    }
   ...
   */
    def times(chars: List[Char]): List[(Char, Int)] = {
      @tailrec
      def calc(charlist: List[Char], acc: List[(Char, Int)]): List[(Char, Int)] = charlist match {
        case Nil => acc
        case x :: xs => calc(xs.filter(_ != x), (x, xs.count(_ == x)):: acc)
      }
      calc(chars, List())
    }
   ...
   */
    def makeOrderedLeafList(freqs: List[(Char, Int)]): List[Leaf] = {
      freqs.sortBy(_._2).map(a => Leaf(char=a._1, weight=a._2))
    }
   ...
   */
    def until(single:  List[CodeTree] => Boolean, combo: List[CodeTree] => List[CodeTree])(trees: List[CodeTree]): CodeTree = {
      if (single(trees)) trees.head
      else
        until(single,combo)(combo(trees))
    }
   ...
   */
    def decode(tree: CodeTree, bits: List[Bit]): List[Char] = {
      def decoding(node: CodeTree, lbits: List[Bit], acc: List[Char]): List[Char] = lbits match {
        case Nil => node match {
          case Leaf(char, _) => acc :+ char
        }
        case x :: xs => node match {
          case Leaf(char, _) => decoding(tree, lbits, acc :+ char)
          case Fork(left, right, _, _) => decoding(if (x == 0) left else right, xs, acc)
        }
      }
