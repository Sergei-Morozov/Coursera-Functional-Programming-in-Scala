...
object Anagrams {
  ...
  def wordOccurrences(w: Word): Occurrences = {
    ...
    val ch_map = group map {case (ch, str) => (ch, str.length)}
    ch_map.toList.sortBy(_._1)
  }
  ...
  def combinations(occurrences: Occurrences): List[Occurrences] =
    (occurrences foldRight List[Occurrences](Nil)) {
      case ((chr, str), acc) => {
        acc ++ (for { combination <- acc; n <- 1 to str }
            yield (chr, n) :: combination
          )
      }
    }
  ...
  def subtract(x: Occurrences, y: Occurrences): Occurrences = {
    ...
    {
      case (map, (chr, str)) => {
        val newStr = map(chr) - str
        if (newStr != 0)
          map updated (chr, newStr)
        else map - chr
      }
    }.toList
  }
